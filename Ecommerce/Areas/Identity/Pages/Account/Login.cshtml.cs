    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var user = _unitOfWork.ApplicationUsers.Get(x => x.Email == Input.Email);
                    int cont = _unitOfWork.ShoppingCarts.GetAll(x => x.ApplicationUserId == user.Id).Count();
                    HttpContext.Session.SetInt32(SD.SessionCart, cont);
